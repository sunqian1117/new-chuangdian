...
import { toBuffer } from "@/utils/convert"
...
export async function resizeImageData(imageData, width, height) {
  ...
  if (window.resizeCanvas == null) {
    window.resizeCanvas = document.createElement('canvas')
  }
  window.resizeCanvas.width = resizeWidth
  window.resizeCanvas.height = resizeHeight
  const ctx = window.resizeCanvas.getContext('2d', {
...
export async function resizeImageDataAndSave(imageData, width, height) {
  const resizeWidth = width >> 0
  const resizeHeight = height >> 0
  const ibm = await window.createImageBitmap(imageData, 0, 0, imageData.width, imageData.height, {
    resizeWidth,
  ...
  // ctx.scale(resizeWidth / imageData.width, resizeHeight / imageData.height)
  ctx.drawImage(ibm, 0, 0)
  window.resizeCanvas.toBlob((blob) => {
    new ImageCompressor(blob, {
      ...
      success(result) {
        ...
        reader.onload = function () {
          ...
          fs.writeFile('/Users/zcp/Desktop/ai/' + ((window.timeName + "|" + name) + '.jpg'), toBuffer(reader.result), error => {
  ...
  //  ctx.getImageData(0, 0, resizeWidth, resizeHeight)
}